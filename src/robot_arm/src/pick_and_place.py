...
class PickAndPlace:
    def __init__(self):
        self.arm = InterbotixRobot(
            robot_name="rx150", mrd=mrd, gripper_pressure=1)
        ...
        self.request_detection_pub = rospy.Publisher(
            '/robot/get_new_point', String, queue_size=1)
        self.target = Point()
    ...
    def move_to_point(self, pt):
        ...
        for guess in custom_guesses:
            _, res = self.arm.set_ee_pose_components(
                x=pt.x, y=pt.y, z=pt.z, custom_guess=guess)
            if(res):
